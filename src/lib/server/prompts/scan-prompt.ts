...
export function getScanRuntimeSystemPrompt(
  ...
  return [
    ...
    "",
    "Character importance and detail extraction rules:",
    "- For every character, set characterImportance to main, major, or minor. This field is required for all characters.",
    ...
    "- Do not default new characters to minor just because prior canon is sparse or missing. Use chapter-local evidence.",
    "- For every character, extract:",
    "  - facts: array of atomic canon facts (see schema). Each fact must include: field, value, evidence, confidence, persistence, sceneLabel, sourceChapterNumber.",
    "- All entity details must be expressed as atomic facts in the facts array. Do not use legacy arrays.",
    "- Preserve all summary text in the summary field; do not replace with generic templates if details are present.",
    "- Only mark a section as Missing / unestablished if the corresponding facts are missing.",
    "",
